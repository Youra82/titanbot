...
def run_titan_backtest(data, params, verbose=True):
    ...
    for i in range(1, len(data)):
        ...
        if status == 'in_trade':
            ...
            # ... (Logik für Liquidation, SL, TP bleibt unverändert) ...
            liquidated = (side == 'long' and current_candle['low'] <= liquidation_price) or (side == 'short' and current_candle['high'] >= liquidation_price)
            if liquidated:
                pnl = - (current_capital * trade_size_pct); current_capital += pnl; trades_count += 1; consecutive_loss_count += 1; verlust_vortrag += abs(pnl); status = 'none'; continue
            stop_hit = (side == 'long' and current_candle['low'] <= sl_price) or (side == 'short' and current_candle['high'] >= sl_price)
            if stop_hit:
                exit_price = sl_price; pnl_pct = (exit_price / entry_price - 1) if side == 'long' else (1 - exit_price / entry_price); trade_pnl = current_capital * trade_size_pct * pnl_pct * leverage; fee = current_capital * trade_size_pct * fee_pct * 2 * leverage; total_pnl = trade_pnl - fee; current_capital += total_pnl; trades_count += 1; consecutive_loss_count += 1; verlust_vortrag += abs(total_pnl); max_adverse_excursion = max(max_adverse_excursion, current_trade_mae); status = 'none'; continue
            tp_hit = (side == 'long' and current_candle['high'] >= tp_price) or (side == 'short' and current_candle['low'] <= tp_price)
            if tp_hit:
                exit_price = tp_price; pnl_pct = (exit_price / entry_price - 1) if side == 'long' else (1 - exit_price / entry_price); trade_pnl = current_capital * trade_size_pct * pnl_pct * leverage; fee = current_capital * trade_size_pct * fee_pct * 2 * leverage; total_pnl = trade_pnl - fee; current_capital += total_pnl; trades_count += 1; wins_count += 1; consecutive_loss_count = 0; verlust_vortrag = 0.0; max_adverse_excursion = max(max_adverse_excursion, current_trade_mae); status = 'none'; continue
    ...
    # +++ NEU: Berechne hypothetische Endkapital-Stände +++
    end_capital_max_lev = 0
    end_capital_rec_lev = 0
    # Diese Extrapolation ist nur sinnvoll, wenn die Basis-Simulation mit 1x Hebel lief.
    if leverage == 1.0 and final_pnl_pct is not None:
        pnl_at_1x_pct = final_pnl_pct
        if max_leverage != float('inf'):
            end_capital_max_lev = start_capital * (1 + (pnl_at_1x_pct / 100) * max_leverage)
        if recommended_leverage != float('inf'):
            end_capital_rec_lev = start_capital * (1 + (pnl_at_1x_pct / 100) * recommended_leverage)
    ...
    return {
        ...
        "end_capital_max_lev": end_capital_max_lev, # <-- NEUER RÜCKGABEWERT
        "end_capital_rec_lev": end_capital_rec_lev  # <-- NEUER RÜCKGABEWERT
    }
