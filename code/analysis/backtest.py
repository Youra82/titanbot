...
def run_titan_backtest(data, params, verbose=True):
    ...
    for i in range(1, len(data)):
        ...
        if status == 'in_trade':
            ...
            if closed_trade:
                trades_count += 1
                trade_dates.append(str(current_candle.name)) # Datum bei Trade-Ende hinzufügen
                status = 'none'
                continue
    ...
    return {
        ...
        "end_capital_max_lev": end_capital_max_lev,
        "end_capital_rec_lev": end_capital_rec_lev,
