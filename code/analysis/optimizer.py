...
def run_titan_optimization(start_date, end_date, symbols, leverage, start_capital, trade_size_pct):
    ...
    strategies_to_run = []
    if choice == len(strategy_names) + 1: strategies_to_run = strategy_names; print("\nINFO: Alle Strategien werden mit Standard-Parametern getestet.")
    elif 1 <= choice <= len(strategy_names): strategies_to_run.append(strategy_names[choice - 1])
    else: print("Ungültige Auswahl. Abbruch."); return
    timeframe_input = input("Zu testende Timeframe(s) eingeben (z.B. 15m 1h 4h): ")
    if not timeframe_input: print("Fehler: Mindestens ein Timeframe ist erforderlich. Abbruch."); return
    ...
    for symbol_short in symbols:
        ...
        for timeframe in timeframes_to_run:
            print("\n" + "="*60); print(f"=== OPTIMIERE: {symbol} auf TIMEFRAME: {timeframe.upper()} ==="); print("="*60)
            ...
            for strategy_name in strategies_to_run:
                print("\n" + "#"*60); print(f"#####  Strategie: {strategy_name.upper()}  #####"); print("#"*60)
                if len(strategies_to_run) > 1: param_grid = parse_default_params(strategy_name)
                else: param_grid = get_user_params(strategy_name)
                signal_func = STRATEGY_CONFIG[strategy_name]['signal_func']
                keys, values = zip(*param_grid.items()); param_combinations = [dict(zip(keys, v)) for v in product(*values)]; total_runs = len(param_combinations)
                proceed = True
                estimated_total_seconds = 0
                if total_runs > 5:
                    pass # Benchmark-Logik
                ...
                params_df_inner = pd.json_normalize(results_df['params'])
                results_df = pd.concat([results_df.drop('params', axis=1), params_df_inner], axis=1)
                sorted_results = results_df.sort_values(by='total_pnl_pct', ascending=False).head(5)
                print(f"\n--- Top 5 Ergebnisse für {strategy_name} auf {timeframe} ---")
                for i, row in sorted_results.reset_index(drop=True).iterrows():
                    param_display = {k: row[k] for k in param_grid.keys()}
                    print(f"  Platz {i+1}: PnL {row['total_pnl_pct']:.2f}%, Trades {int(row['trades_count'])}, Params: {param_display}")
