...
def _generate_trades_excel(final_sim, capital):
    ...
    for i, t in enumerate(trade_history):
        ...
        symbol = t.get('symbol', '')
        tf     = t.get('timeframe', '')
        coin   = symbol.split('/')[0] if symbol else 'â'
        ...
        rows.append({
            ...
            'SL %':               f"{sl_pct:.3f}%" if sl_pct else 'â',
            'Entry':              entry,
            'Exit':               exit_p,
    ...
    for label, value in [
        ('Trades gesamt', total),
        ('Win-Rate',      f"{wins / total * 100:.1f}%" if total else 'â'),
        ('PnL',           f"{pnl_pct:+.1f}%"),
        ('Endkapital',    f"{rows[-1]['Gesamtkapital']:.2f} USDT" if rows else 'â'),
    ]:
    ...
    if bot_token and chat_id:
        caption = (f"TitanBot Trades â {total} Trades | "
        ...
        print(f"  {YELLOW}Telegram nicht konfiguriert â nur lokal gespeichert.{NC}")
...
def run_shared_mode(is_auto: bool, start_date, end_date, start_capital, target_max_dd: float, warmup_date=None):
    ...
    if not is_auto:
        ...
        selection = input("\nWelche Strategien simulieren? (Zahlen mit Komma oder 'alle'): ")
        try:
        ...
        except (ValueError, IndexError): print("Ungueltige Auswahl. Breche ab."); return
    else:
        selected_files = available_strategies

    strategies_data = {}
    print("\nLade Daten fuer gewaehlte Strategien...")
    for filename in selected_files:
        try:
            ...
            if not data.empty:
                ...
                print(f"WARNUNG: Keine Daten fuer {filename}. Ignoriert.")
        except Exception as e:
            print(f"FEHLER bei {filename}: {e}")

    if not strategies_data:
        print("Keine Daten geladen. Breche ab."); return
    ...
    try:
        if is_auto:
            # Risk-Level-Optimierung (wie dnabot): 1.0%--5.0% in 0.5%-Schritten
            risk_levels = [r / 10 for r in range(10, 55, 5)]
            print(f"\nSuche optimales Risiko ({risk_levels[0]}%--{risk_levels[-1]}%, MaxDD <= {target_max_dd:.0f}%)...")
            best_risk_pct  = 1.0
            best_results   = None
            best_portfolio = None
            best_calmar    = -999.0
            ...
            for risk_pct in risk_levels:
                ...
                for fname, sd in strategies_data.items():
                    entry = dict(sd)
                    entry["risk_params"] = dict(sd["risk_params"])
                    entry["risk_params"]["risk_per_trade_pct"] = risk_pct
                    sd_risk[fname] = entry

                r = run_portfolio_optimizer(start_capital, sd_risk, start_date, end_date, target_max_dd)
                if not r or not r.get("final_result"):
                    continue
                m = r["final_result"]
                if m.get("max_drawdown_pct", 100) > target_max_dd:
                    continue
                pnl    = m.get("total_pnl_pct", 0)
                dd     = max(m.get("max_drawdown_pct", 0.001), 0.001)
                calmar = pnl / dd
                colour = GREEN if pnl > 0 else YELLOW
                print(f"  risk={risk_pct:.1f}%  PnL={colour}{pnl:+.1f}%{NC}  DD={dd:.1f}%  Calmar={calmar:.2f}")
                if calmar > best_calmar:
                    best_calmar    = calmar
                    best_risk_pct  = risk_pct
                    best_results   = r
                    best_portfolio = r.get("optimal_portfolio", [])

            if best_results and best_results.get("final_result"):
                final_report         = best_results["final_result"]
                portfolio_files_used = best_portfolio or selected_files

                print(f"\n{GREEN}Bestes Risiko: {best_risk_pct}%  (Calmar: {best_calmar:.2f}){NC}")
                print("\n=======================================================")
                print("     Ergebnis der automatischen Portfolio-Optimierung")
                print("=======================================================")
                print(f"Zeitraum:           {start_date} bis {end_date}")
                print(f"Startkapital:       {start_capital:.2f} USDT")
                print(f"Optimales Risiko:   {best_risk_pct}% pro Trade")
                print(f"Max Drawdown:       <= {target_max_dd:.2f}%")
                print(f"\nOptimales Portfolio ({len(portfolio_files_used)} Strategien):")
                ...
                # Bestes Risiko + Portfolio automatisch in settings.json schreiben
                settings_path = os.path.join(PROJECT_ROOT, "settings.json")
                try:
                    with open(settings_path, "r") as f:
                        settings = json.load(f)
                    settings.setdefault("risk_settings", {})["risk_per_trade_pct"] = best_risk_pct
                    new_active = [
                        {"symbol": strategies_data[fn]["symbol"],
                         "timeframe": strategies_data[fn]["timeframe"],
                         "active": True}
                        for fn in portfolio_files_used if fn in strategies_data
                    ]
                    settings["live_trading_settings"]["active_strategies"] = new_active
                    with open(settings_path, "w") as f:
                        json.dump(settings, f, indent=4)
                    print(f"\n{GREEN}settings.json aktualisiert:{NC}")
                    print(f"  risk_per_trade_pct = {best_risk_pct}%")
                    print(f"  {len(new_active)} aktive Strategien eingetragen")
                except Exception as e:
                    print(f"\n{YELLOW}WARN: settings.json Fehler: {e}{NC}")
            else:
                print(f"\nKein Portfolio gefunden das Max Drawdown <= {target_max_dd:.2f}% einhaelt.")
        ...
        # --- Manuelle Simulation ---
            ...
            if results:
                ...
                print(f"Gesamt PnL:         {results['end_capital'] - results['start_capital']:+.2f} USDT ({results['total_pnl_pct']:.2f}%)")
                print(f"Anzahl Trades:      {results['trade_count']}")
                print(f"Win-Rate:           {results['win_rate']:.2f}%")
                dd_date = results['max_drawdown_date'].strftime('%Y-%m-%d') if results['max_drawdown_date'] else 'N/A'
                print(f"Portfolio Max DD:   {results['max_drawdown_pct']:.2f}% am {dd_date}")
